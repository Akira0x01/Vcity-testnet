...
echo $(BIN debug addr $($BIN keys show validator -a --home $DATA_DIR --keyring-backend test)) >> $NODE_INFO_FILE
echo $(BIN debug addr $(grep "Address hex:" "$NODE_INFO_FILE" | awk -F'[: ]' '{print $4}')) >> $NODE_INFO_FILE
# node id
